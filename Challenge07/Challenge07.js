...
const objLat = (obj) => {
    ...
    let firstName = obj.firstName[0].toUpperCase() + obj.firstName.substring(1);
    let lastName = obj.lastName[0].toUpperCase() + obj.lastName.substring(1);

    let result = `my name is ${firstName} ${lastName} I am ${obj.age} YO, and I love ${obj.hobby}.`
    return result;
};
...
const applicationsStatics = (arr) => {
    ...
    arr.forEach((obj) => {
        ...
        if (obj.tech === 'JS') {
            ...
            count++;
        } else if (obj.tech === 'Python') {
            result.python_Devs += 1;
            count++;
        } else if (obj.tech === 'Java') {
            result.java_Devs += 1;
            count++;
        } else if (obj.tech === '.Net') {
            result.dotNet_Devs += 1;
            count++;
        }
    });
    result.totalApplicants = count;
    return result;
};
